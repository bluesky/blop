...
from blop.queueserver import ConsumerCallback, QueueserverClient, QueueserverOptimizationRunner
...
def test_consumer_callback_caches_start_and_calls_on_stop():
    ...
    start_doc = {"uid": "test-uid", "blop_correlation_uid": "123", "time": 123}
...
def test_consumer_callback_clears_cache_after_stop():
    ...
    mock_callback = MagicMock()
    callback = ConsumerCallback(callback=mock_callback)
    start_doc = {"uid": "test-uid", "blop_correlation_uid": "123"}
...
def test_runner_run_submits_suggestions_to_qserver(mock_optimization_problem):
